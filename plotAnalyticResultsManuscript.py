...
def main():
    ...
    for metric in metrics:
        ...
        drawMetricPerBudget(metric, [(0, 3), (1, 3), (2 , 3), (1, 2, 3), (0, 1, 2, 3)], restultPath, restultPath + metric + '/', drawAll = False)     
...
def drawFinalSamplesPerBudget(stages, restultPath, outPath):
    ...
    for fileName in fileList:
        ...
        # # if fileName != './results/data1101001000_manuscript/cov9.pickle':
        # if fileName != './results/data1101001000_manuscript/cov10.pickle':
        #     continue
        ...
        for stageSetup in stages:
            for algorithm in data[''.join(map(str, stageSetup))][0]['algorithms']:
                ...
                lStyle1['$[S_' + ',S_'.join(map(str, np.array(stageSetup )+1)) + ']$'] = getLineColor3(stageSetup)
        ...
        for i in range(3):
            ...
            plt.xscale("log")
            if i == 2:
                plt.xlabel('log(Total resource budget)')
            if i == 1:
        ...
        bestDf.plot(style =  bestLineColor, linewidth = 0.7, alpha=0.7, ax=ax1, markevery=10, markersize=2)
        # bestDf.plot(figsize=(4, 3.5), style =  bestLineColor, linewidth = 1, alpha=0.7, ax=ax)
        # baselineDf.plot(figsize=(4, 3.5), style =  bestLineColor, linewidth = 0.8, linestyle = 'dashed', alpha=0.5, ax=ax)
        # plt.xlabel('Total computational budget $C$')
        # plt.ylabel('The number of potential samples $\left|\mathbb{Y}\\right|$')
        ax1.legend(loc='center', bbox_to_anchor=(0.5, -0.8))
        ax1.set_xlabel('Total resource budget')
        ax1.set_ylabel('The number of potential candidates')
        # plt.xlim(-5000000, 5000000+10**8 )
        ax1.set_ylim(0, 105)
        ax1.grid(True)
        ax1.axvline(x = 1000*(10**5), ls = '--', color = 'k', linewidth = 0.8)
        ax1.axhline(y = 100, ls = '--', color = 'k', linewidth = 0.8)
        # np.concatenate((np.array([-.05*10**8]), np.arange(0, 1.01*10**8, step=10**7), np.array([1.05*10**8])))
        # ax1.set_xticks(np.arange(0, 1.01*10**8, step=10**7))
        # ax1.set_yticks(np.arange(0, 110, step=20))

        # bestDf = bestDf.set_index(bestDf.index.values / (5000000+10**8))
        # bestDf = bestDf.replace(bestDf.values / 100) 
        # bestDf.loc[:, :]=(bestDf.values / 100)
        bestDf.plot(figsize=(9, 2), style =  bestLineColor, linewidth = 0.7, alpha=0.7, markevery=10, markersize=2, ax=ax2)
        # bestDf.plot(figsize=(4, 3.5), style =  bestLineColor, linewidth = 1, alpha=0.7, ax=ax)
        # baselineDf.plot(figsize=(4, 3.5), style =  bestLineColor, linewidth = 0.8, linestyle = 'dashed', alpha=0.5, ax=ax)
        # plt.xlabel('Total computational budget $C$')
        # plt.ylabel('The number of potential samples $\left|\mathbb{Y}\\right|$')
        ax2.legend(loc='center', bbox_to_anchor=(0.5, -0.8))
        ax2.set_title('A. $\\rho = 0.8$ B. $\\rho = 0.3$')
        ax2.set_xlabel('log(Total resource budget)')
        ax2.set_ylabel('log(The number of potential candidates)')
        ax2.set_ylim(1, 125)
        # plt.xlim(-5000000, 5000000+10**8 )
        # plt.xlim(0, 0.3)
        # ax2.set_ylim(0, 1.05)
        ax2.grid(True)
        ax2.axvline(x = 1000*(10**5), ls = '--', color = 'k', linewidth = 0.8)
        ax2.axhline(y = 100, ls = '--', color = 'k', linewidth = 0.8)
        # np.concatenate((np.array([-.05*10**8]), np.arange(0, 1.01*10**8, step=10**7), np.array([1.05*10**8])))
        # plt.xticks(np.arange(0, 1.01*10**8, step=10**7))
        # plt.yticks(np.arange(0, 110, step=20))
        ax2.set_yscale("log")
        ax2.set_xscale("log")
        plt.savefig(outPath + 'all/' + fileName.replace(restultPath, "").replace(".pickle", "") + targetAlgorithm + "_best_v3.pdf", bbox_inches='tight')
        plt.close()
